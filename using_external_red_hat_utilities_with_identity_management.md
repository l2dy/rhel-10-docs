...
[Legal Notice](#idm139830411581808)

**Abstract**

Administrators can integrate services and Red Hat products in a Red Hat Enterprise Linux Identity Management (IdM) domain. This includes services, such as Samba, Ansible, and automount, and also products, such as OpenShift Container Platform, OpenStack, and Satellite. IdM users can then access these services and products.
...
<h2 id="idm-integration-with-red-hat-products">Chapter 1. IdM integration with Red Hat products</h2>
...
<h2 id="using-external-identity-providers-to-authenticate-to-idm">Chapter 2. Using external identity providers to authenticate to IdM</h2>
...
<h3 id="the-benefits-of-connecting-idm-to-an-external-idp">2.1. The benefits of connecting IdM to an external IdP</h3>
...
<h3 id="how-idm-incorporates-logins-via-external-idps">2.2. How IdM incorporates logins via external IdPs</h3>
...
<h3 id="creating-a-reference-to-an-external-identity-provider">2.3. Creating a reference to an external identity provider</h3>

You can create IdP references in IdM to connect external identity providers (IdPs) to your Identity Management (IdM) environment. You can create a reference called **my-keycloak-idp** to an IdP based on the Keycloak template. For more reference templates, see [Example references to different external IdPs in IdM](#example-references-to-different-external-idps-in-idm "2.4. Example references to different external IdPs in IdM").
...
- [Example references to different external IdPs in IdM](#example-references-to-different-external-idps-in-idm "2.4. Example references to different external IdPs in IdM")
- [Options for the ipa idp-* commands to manage external identity providers in IdM](#options-for-the-ipa-idp-commands-to-manage-external-identity-providers-in-idm "2.5. Options for the ipa idp-* commands to manage external identity providers in IdM")
- [The --provider option in the ipa idp-* commands](#the-provider-option-in-the-ipa-idp-commands "2.10. The --provider option in the ipa idp-* commands")

<h3 id="example-references-to-different-external-idps-in-idm">2.4. Example references to different external IdPs in IdM</h3>
...
- [Creating a reference to an external identity provider](#creating-a-reference-to-an-external-identity-provider "2.3. Creating a reference to an external identity provider")
- [Options for the ipa idp-* commands to manage external identity providers in IdM](#options-for-the-ipa-idp-commands-to-manage-external-identity-providers-in-idm "2.5. Options for the ipa idp-* commands to manage external identity providers in IdM")
- [The --provider option in the ipa idp-* commands](#the-provider-option-in-the-ipa-idp-commands "2.10. The --provider option in the ipa idp-* commands")
...
<h3 id="options-for-the-ipa-idp-commands-to-manage-external-identity-providers-in-idm">2.5. Options for the ipa idp-* commands to manage external identity providers in IdM</h3>
...
<h3 id="managing-references-to-external-idps">2.6. Managing references to external IdPs</h3>
...
- You can authenticate as the IdM admin account.
- You have created a reference to an external IdP in IdM. See [Creating a reference to an external identity provider](#creating-a-reference-to-an-external-identity-provider "2.3. Creating a reference to an external identity provider").
...
<h3 id="enabling-an-idm-user-to-authenticate-via-an-external-idp">2.7. Enabling an IdM user to authenticate via an external IdP</h3>
...
<h3 id="retrieving-an-idm-ticket-granting-ticket-as-an-external-idp-user">2.8. Retrieving an IdM ticket-granting ticket as an external IdP user</h3>
...
- You have created a reference to an external IdP in IdM. See [Creating a reference to an external identity provider](#creating-a-reference-to-an-external-identity-provider "2.3. Creating a reference to an external identity provider").
- You have associated an external IdP reference with the user account. See [Enabling an IdM user to authenticate via an external IdP](#enabling-an-idm-user-to-authenticate-via-an-external-idp "2.7. Enabling an IdM user to authenticate via an external IdP").
...
<h3 id="logging-in-to-an-idm-client-via-ssh-as-an-external-idp-user">2.9. Logging in to an IdM client via SSH as an external IdP user</h3>
...
<h3 id="the-provider-option-in-the-ipa-idp-commands">2.10. The --provider option in the ipa idp-* commands</h3>
...
<h3 id="using-external-identity-providers-to-authenticate-to-idm">2.11. Additional resources</h3>
...
<h2 id="setting-up-samba-on-an-idm-domain-member">Chapter 3. Setting up Samba on an IdM domain member</h2>
...
<h3 id="preparing-the-idm-domain-for-installing-samba-on-domain-members">3.2. Preparing the IdM domain for installing Samba on domain members</h3>
...
<h3 id="installing-and-configuring-a-samba-server-on-an-idm-client">3.3. Installing and configuring a Samba server on an IdM client</h3>
...
- The IdM domain is prepared as described in [Preparing the IdM domain for installing Samba on domain members](#preparing-the-idm-domain-for-installing-samba-on-domain-members "3.2. Preparing the IdM domain for installing Samba on domain members").
...
<h3 id="manually-adding-an-id-mapping-configuration-if-idm-trusts-a-new-domain">3.4. Manually adding an ID mapping configuration if IdM trusts a new domain</h3>
...
<h3 id="setting-up-samba-on-an-idm-domain-member">3.5. Additional resources</h3>
...
<h2 id="migrating-from-nis-to-identity-management">Chapter 4. Migrating from NIS to Identity Management</h2>
...
<h3 id="enabling-nis">4.1. Enabling NIS in IdM</h3>
...
<h3 id="nis-import-users">4.2. Migrating user entries from NIS to IdM</h3>
...
- [NIS is enabled in IdM.](#enabling-nis "4.1. Enabling NIS in IdM")
- The NIS server is enrolled into IdM.
- You have [ID ranges](https://docs.redhat.com/en/documentation/red_hat_enterprise_linux/10/html/managing_idm_users_groups_hosts_and_access_control_rules/adjusting-id-ranges-manually) that can store UIDs of importing users.
...
<h3 id="nis-import-groups">4.3. Migrating user group from NIS to IdM</h3>
...
<h3 id="nis-import-hosts">4.4. Migrating host entries from NIS to IdM</h3>
...
<h3 id="nis-import-netgroups">4.5. Migrating netgroup entries from NIS to IdM</h3>
...
<h3 id="nis-import-automount">4.6. Migrating automount maps from NIS to IdM</h3>
...
<h2 id="using-ansible-to-automount-nfs-shares-for-idm-users">Chapter 5. Using Ansible to automount NFS shares for IdM users</h2>
...
<h3 id="autofs-and-automount-in-idm">5.1. Autofs and automount in IdM</h3>
...
<h3 id="setting-up-an-nfs-server-with-kerberos-in-a-rhel-identity-management-domain">5.2. Setting up an NFS server with Kerberos in an Identity Management domain</h3>
...
<h3 id="configuring-automount-locations-maps-and-keys-in-idm-by-using-ansible">5.3. Configuring automount locations, maps, and keys in IdM by using Ansible</h3>
...
<h3 id="using-ansible-to-add-idm-users-to-a-group-that-owns-nfs-shares">5.4. Using Ansible to add IdM users to a group that owns NFS shares</h3>
...
<h3 id="configuring-automount-on-an-idm-client">5.5. Configuring automount on an IdM client</h3>
...
<h3 id="verifying-that-an-idm-user-can-access-nfs-shares-on-an-idm-client\_ansible-automount">5.6. Verifying that an IdM user can access NFS shares on an IdM client</h3>
...
- You have [set up an NFS server with Kerberos on an IdM host](#setting-up-an-nfs-server-with-kerberos-in-a-rhel-identity-management-domain "5.2. Setting up an NFS server with Kerberos in an Identity Management domain").
- You have [configured automount locations, maps, and mount points in IdM](#configuring-automount-locations-maps-and-keys-in-idm-by-using-ansible "5.3. Configuring automount locations, maps, and keys in IdM by using Ansible") in which you configured how IdM users can access the NFS share.
- You have [used Ansible to add IdM users to the developers group that owns the NFS shares](#using-ansible-to-add-idm-users-to-a-group-that-owns-nfs-shares "5.4. Using Ansible to add IdM users to a group that owns NFS shares").
- You have [configured automount on the IdM client](#configuring-automount-on-an-idm-client "5.5. Configuring automount on an IdM client").
...
<h2 id="idm139830411581808">Legal Notice</h2>
