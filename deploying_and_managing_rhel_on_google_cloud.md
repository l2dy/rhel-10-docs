...
[Legal Notice](#idm139967677608208)

**Abstract**

To use Red Hat Enterprise Linux (RHEL) on Google Cloud, you can create and deploy RHEL system images.

This also involves:

- Registering, deploying, and provisioning RHEL images on Google Cloud
- Managing networking configurations for RHEL Google Cloud instances
...
<h2 id="preparing-and-uploading-custom-gce-images-to-gcp">Chapter 2. Preparing and uploading custom GCE images to Google Cloud</h2>
...
<h3 id="configuring-and-uploading-a-gce-image-to-gcp-by-using-the-cli">2.1. Configuring and uploading a gce image to Google Cloud by using the CLI</h3>
...
<h3 id="how-rhel-image-builder-sorts-the-authentication-order-of-different-gcp-credentials">2.2. How RHEL image builder sorts the authentication order of different Google Cloud credentials</h3>
...
<h2 id="deploying-a-rhel-image-as-a-google-compute-engine-instance-on-gcp">Chapter 3. Deploying a RHEL image as a Google Compute Engine instance on Google Cloud</h2>
...
- Use the RHEL image builder. For instructions, see [Preparing and uploading custom GCE images to Google Cloud](#preparing-and-uploading-custom-gce-images-to-gcp "Chapter 2. Preparing and uploading custom GCE images to Google Cloud").
- Manually create and configure a [supported format image](https://cloud.google.com/compute/docs/import/importing-virtual-disks#supported-formats). This is a more complicated process but offers more granular customization options. For details, see the following sections.

Note

For a list of Red Hat product certifications for Google Cloud, see [Red Hat on Google Cloud](https://catalog.redhat.com/cloud/detail/82585).
...
- You have [signed up and set up a Google Cloud account](https://cloud.google.com/free).
...
- [Create and start a Compute Engine instance](https://cloud.google.com/compute/docs/instances/create-start-instance#creating_an_instance_from_a_custom_image)
...
<h3 id="uploading-and-running-a-rhel-instance-on-gcp">3.3. Uploading and running a RHEL instance on Google Cloud</h3>

To run your RHEL instance on Google Cloud, you need to configure and upload a RHEL image to Google Cloud.

<h4 id="installing-the-google-cloud-sdk">3.3.1. Installing the Google Cloud SDK</h4>
...
- You have downloaded, extracted, and initialized [the Google Cloud SDK](https://cloud.google.com/sdk/docs/install-sdk#rpm).
...
- [gcloud project create](https://cloud.google.com/sdk/gcloud/reference/projects/create)
- [gcloud command-line tool overview](https://cloud.google.com/sdk/gcloud/)

<h4 id="creating-a-new-project-on-gcp">3.3.2. Creating a new project on Google Cloud</h4>
...
- You have an account on [Google Cloud](https://console.cloud.google.com/).

**Procedure**

1. Launch the [Google Cloud Console](https://console.cloud.google.com/).
...
- [Creating and Managing Resources in Google Cloud](https://cloud.google.com/resource-manager/docs/creating-managing-projects)

<h4 id="creating-and-uploading-a-rhel-image-on-google-cloud-storage">3.3.3. Creating and uploading a RHEL image on Google Cloud Storage</h4>
...
2. Create a storage bucket:
   ...
   Alternatively, you can use the Google Cloud Console to create a bucket. For details, see [Create a bucket](https://cloud.google.com/storage/docs/quickstart-console#create_a_bucket).
3. Specify the image name that you want to create, the existing bucket name, and the name of the image:
   ...
   *gcloud compute images create my-image-name --source-uri gs://__<example_bucket_name>__/disk.raw.tar.gz*
   ...
   Alternatively, you can use the Google Cloud Console to create an image. See [Creating, deleting, and deprecating custom images](https://cloud.google.com/compute/docs/images/create-delete-deprecate-private-images) for more information.
...
- [Manually importing virtual disks](https://cloud.google.com/compute/docs/import/import-existing-image)
- [Choosing an import method](https://cloud.google.com/compute/docs/import/)
- [Installing the gsutil command-line tool](https://cloud.google.com/storage/docs/gsutil_install)

<h4 id="launching-and-connecting-to-a-rhel-google-compute-engine-instance">3.3.4. Launching and connecting to a RHEL Google Compute Engine instance</h4>
...
01. From the [Google Cloud Console Dashboard page](https://console.cloud.google.com/home/dashboard), click the **Navigation** menu to the left of the Google **Cloud Console banner** and select **Compute Engine** and then select **Images**.
02. Select your image.
03. Click **Create Instance**.
04. On the **Create an instance** page, enter a **Name** for your instance.
05. Choose a **Region** and **Zone**.
06. Choose a **Machine configuration** that meets or exceeds the requirements of your workload.
07. Ensure that **Boot disk** specifies the name of your image.
08. Optional: Under **Firewall**, select **Allow HTTP traffic** or **Allow HTTPS traffic**.
09. Click **Create**.
...
11. Click the **Navigation** menu to the left of the Google **Cloud Console banner** and select **Compute Engine** and then select **VM instances**.
    ...
    The example creates a VM instance named `myinstance3` in zone `us-central1-a` based upon the existing image `test-iso2-image`. For details, see [gcloud compute instances create](https://cloud.google.com/sdk/gcloud/reference/compute/instances/create).
12. Use the `ssh-keygen` utility to generate an SSH key pair to use with GCE by using the public IP address:
    ...
    # ssh-keygen -t rsa -f ~/.ssh/google_compute_engine.pub
    ```
13. From the [Google Cloud Console Dashboard page](https://console.cloud.google.com/home/dashboard), click the **Navigation** menu to the left of the Google **Cloud Console banner** and select **Compute Engine** and then select **Metadata**.
...
16. Connect to the instance:
    ...
    Or, you can run the `gcloud compute config-ssh` command to populate the config file with aliases for instances. The aliases allow simple SSH connections by instance name. For details, see [gcloud compute config-ssh](https://cloud.google.com/sdk/gcloud/reference/compute/config-ssh).
...
<h3 id="attaching-red-hat-subscriptions">3.4. Attaching Red Hat subscriptions</h3>
...
<h3 id="setting-up-automatic-registration-on-gcp-gold-images">3.5. Setting up automatic registration on Google Cloud gold images</h3>
...
- RHEL gold images are available to you in Google Cloud. For instructions, see [Using gold images on Google](https://docs.redhat.com/en/documentation/subscription_central/1-latest/html/getting_started_with_rhel_system_registration/red-hat-cloud-access-program-overview_#using-gold-images-on-google_using-gold-images-on-azure).
...
1. Use the gold image to create a RHEL VM in your Google Cloud instance. For instructions, see [Uploading and running a RHEL instance on Google Cloud](#uploading-and-running-a-rhel-instance-on-gcp "3.3. Uploading and running a RHEL instance on Google Cloud").
...
<h2 id="configuring-a-red-hat-high-availability-cluster-on-gcp">Chapter 4. Configuring a Red Hat High Availability Cluster on Google Cloud</h2>
...
To configure a Red Hat HA cluster on Google Cloud that uses Google Compute Engine (GCE) instances as cluster nodes, see the following sections. You have several options for obtaining RHEL images for the cluster. For details, see [Available RHEL image types for public cloud](#available-rhel-image-types-for-public-cloud "3.1. Available RHEL image types for public cloud").

Before you begin, make sure to complete following steps:

- You have created a Red Hat account. For details, see [Red Hat account](https://www.redhat.com/wapps/ugc/register.html).
- You have signed up and set up a Google Cloud account. For details, see [Google Cloud account](https://cloud.google.com/free).
...
- Your project should have a service account that belongs to a VM instance and not an individual user. For details, see [Service accounts](https://cloud.google.com/iam/docs/understanding-service-accounts).
- You can use the default service account instead of creating a separate service account. For details, see [Using the Compute Engine Default Service Account](https://cloud.google.com/compute/docs/access/create-enable-service-accounts-for-instances#using_the_compute_engine_default_service_account).
...
<h3 id="required-system-packages">4.2. Required system packages</h3>
...
By confirming installation of the mentioned system packages, you can now follow the deployment steps in [Deploying a RHEL instance by using a custom base image](#deploying-a-rhel-instance-by-using-a-custom-base-image "3.2. Deploying a RHEL instance by using a custom base image")

<h3 id="creating-a-custom-virtual-private-cloud-network-and-subnet">4.3. Creating a custom virtual private cloud network and subnet</h3>

You must configure a custom virtual private cloud (VPC) network and subnet to enable High Availability (HA) for your cluster.

**Prerequisites**

- You have installed [the Google Cloud SDK](#installing-the-google-cloud-sdk "3.3.1. Installing the Google Cloud SDK").
- You have created [a new Google Cloud project](#creating-a-new-project-on-gcp "3.3.2. Creating a new project on Google Cloud").
...
<h3 id="creating-and-configuring-a-base-gcp-instance">4.4. Creating and configuring a base Google Cloud instance</h3>
...
01. Create an image from the compressed file in the bucket.
    ...
    rhel-76-server  rhel-ha-testing-on-gcp                      READY
    ```
02. Create a template instance from the image. The minimum size required for a base RHEL instance is n1-standard-2. See [gcloud compute instances create](https://cloud.google.com/sdk/gcloud/reference/compute/instances/create) for additional configuration options.
    ...
    rhel-76-server-base-instance  us-east1-bn1-standard-2          10.10.10.3   192.227.54.211  RUNNING
    ```
03. Connect to the instance with an SSH terminal session.
...
04. Update the RHEL software.
    ...
    5. Run the `dnf update` command.
       ...
       # dnf update -y
       ```
05. Install the Google Cloud Linux Guest Environment on the running instance (in-place installation).
    
    See [Install the guest environment in-place](https://cloud.google.com/compute/docs/images/install-guest-environment#in_place) for instructions.
06. Select the **CentOS/RHEL** option.
07. Copy the command script and paste it to the terminal to run the script immediately.
08. Make the following configuration changes to the instance. These changes are based on Google Cloud recommendations for custom images. See [gcloudcompute images list](https://cloud.google.com/sdk/gcloud/reference/compute/images/list) for more information.
...
<h3 id="creating-a-snapshot-image">4.5. Creating a snapshot image</h3>
...
<h3 id="creating-an-ha-node-template-instance-and-ha-nodes">4.6. Creating an HA node template instance and HA nodes</h3>
...
<h3 id="installing-the-high-availability-packages-and-agents-for-gcp">4.7. Installing the high availability packages and agents for Google Cloud</h3>
...
<h3 id="creating-a-high-availability-cluster">4.8. Creating a high availability cluster</h3>
...
<h3 id="configuring-fencing">4.9. Configuring fencing in a Red Hat High Availability cluster</h3>
...
<h4 id="displaying-fence-agents">4.9.1. Displaying available fence agents and their options</h4>
...
<h4 id="creating-fence-devices">4.9.2. Creating a fence device</h4>
...
<h4 id="general-fence-device-properties">4.9.3. General properties of fencing devices</h4>
...
<h4 id="fence-delays">4.9.4. Fencing delays</h4>
...
<h4 id="testing-fence-devices">4.9.5. Testing a fence device</h4>
...
<h4 id="configuring-fencing-levels">4.9.6. Configuring fencing levels</h4>
...
<h4 id="removing-a-fence-level">4.9.7. Removing a fence level</h4>
...
<h4 id="clearing-fence-levels">4.9.8. Clearing fence levels</h4>
...
<h4 id="verifying-nodes-and-devices-in-fence-levels\_general-fence">4.9.9. Verifying nodes and devices in fence levels</h4>
...
<h4 id="specifying-nodes-in-fencing-topology">4.9.10. Specifying nodes in fencing topology</h4>
...
<h4 id="configuring-fencing-for-redundant-power">4.9.11. Configuring fencing for redundant power supplies</h4>
...
<h4 id="administering-fence-devices">4.9.12. Administering fence devices</h4>
...
<h5 id="displaying\_configured\_fence\_devices">4.9.12.1. Displaying configured fence devices</h5>
...
<h5 id="exporting\_fence\_devices\_as\_pcs\_commands">4.9.12.2. Exporting fence devices as pcs commands</h5>
...
<h5 id="exporting\_fence\_level\_configuration">4.9.12.3. Exporting fence level configuration</h5>
...
<h5 id="modifying\_and\_deleting\_fence\_devices">4.9.12.4. Modifying and deleting fence devices</h5>
...
<h5 id="manually\_fencing\_a\_cluster\_node">4.9.12.5. Manually fencing a cluster node</h5>
...
<h5 id="disabling\_a\_fence\_device">4.9.12.6. Disabling a fence device</h5>
...
<h5 id="preventing\_a\_node\_from\_using\_a\_fencing\_device">4.9.12.7. Preventing a node from using a fencing device</h5>
...
<h3 id="configuring-acpi-for-use-with-integrated-fence-devices">4.10. Configuring ACPI for use with integrated fence devices</h3>
...
<h4 id="disabling-acpi-soft-off-with-the-bios">4.10.1. Disabling ACPI Soft-Off with the BIOS</h4>
...
5. Verify that the node turns off immediately when fenced. For information about testing a fence device, see [Testing a fence device](#testing-fence-devices "4.9.5. Testing a fence device").
...
<h4 id="disabling-acpi-soft-off-in-the-logind-conf-file">4.10.2. Disabling ACPI Soft-Off in the logind.conf file</h4>
...
1. Verify that the node turns off immediately when fenced. For information about testing a fence device, see [Testing a fence device](#testing-fence-devices "4.9.5. Testing a fence device").

<h4 id="disabling-acpi-completely-in-the-grub-2-file">4.10.3. Disabling ACPI completely in the GRUB 2 file</h4>
...
<h3 id="configuring-the-virtual-ip-management-resource-agent">4.11. Configuring the virtual IP management resource agent</h3>
...
<h4 id="configuring-the-primary-subnet-address-range">4.11.1. Configuring the primary subnet address range</h4>
...
<h4 id="configuring-the-secondary-subnet-address-range">4.11.2. Configuring the secondary subnet address range</h4>
...
- [You have created a custom network and a subnet](https://access.redhat.com/articles/3479821#header41)
- Optional: You have installed Google Cloud SDK. For instructions, see [Installing the Google Cloud SDK](#installing-the-google-cloud-sdk "3.3.1. Installing the Google Cloud SDK").
...
<h4 id="configuring-the-virtual-ip-management-resource-agent">4.11.3. Additional resources</h4>

- [Support Policies for Red Hat High Availability clusters - Transport Protocols](https://access.redhat.com/articles/3071021)
- [Exploring Red Hat High Availability’s Components, Concepts, and Features - Overview of Transport Protocols](https://access.redhat.com/articles/2679211)
- [Design Guidance for Red Hat High Availability Clusters - Selecting the Transport Protocol](https://access.redhat.com/articles/2679251)

<h2 id="configuring-rhel-on-google-cloud-with-secure-boot">Chapter 5. Configuring RHEL on Google Cloud with Secure Boot</h2>
...
<h3 id="types\_of\_rhel\_images\_on\_google\_cloud">5.1. Types of RHEL images on Google Cloud</h3>
...
<h3 id="configuring-a-rhel-instance-from-a-custom-rhel-image-with-secure-boot">5.3. Configuring a RHEL instance from a custom RHEL image with Secure Boot</h3>
...
- You have created and uploaded a RHEL Google Cloud image. For details, see [Uploading images to Google Cloud](https://docs.redhat.com/en/documentation/red_hat_enterprise_linux/10/html/deploying_and_managing_rhel_on_google_cloud_platform/preparing-and-uploading-custom-gce-images-to-gcp).
- You have installed the following packages:
  ...
  - `python3-virt-firmware`
    ...
    1. You have installed the `google-cloud-cli` utility. For details, see [installing gcloud CLI on RHEL](https://cloud.google.com/sdk/docs/install#rpm).
...
8. Launch the instance of an `example-rhel-10-efi-image` image with the `Turn on Security Boot` feature from the Google Cloud console.
   ...
   ![Secure Boot for RHEL Google Cloud instance](images/gcp-secure-boot-1cf8732999e9.png) ![Secure Boot for RHEL Google Cloud instance](images/gcp-secure-boot-1cf8732999e9.png)
...
<h3 id="configuring-a-rhel-instance-on-the-google-cloud-marketplace-with-secure-boot">5.4. Configuring a RHEL instance on the Google Cloud Marketplace with Secure Boot</h3>
...
- You have an account on Google Cloud. For details, see [Google Cloud](https://console.cloud.google.com/).
...
<h2 id="configuring-a-rhel-instance-on-google-cloud-with-intel-tdx">Chapter 7. Configuring a RHEL instance on Google Cloud with Intel TDX</h2>
...
- You have installed the `google-cloud-cli` utility. For instructions, see [Installing gcloud CLI on RHEL](https://cloud.google.com/sdk/docs/install#rpm).
- You have launched the instance only from a supported Google Cloud instance type. For details, see [Supported Google Cloud instance types for TDX](https://cloud.google.com/confidential-computing/confidential-vm/docs/supported-configurations#machine-type-cpu-zone).
...
<h2 id="configuring-a-rhel-instance-on-google-cloud-with-amd-sev-snp">Chapter 9. Configuring a RHEL instance on Google Cloud with AMD SEV SNP</h2>
...
- You have installed the `openssh` and `openssh-clients` packages.
- You have installed the `google-cloud-cli` utility. For details, see [installing gcloud CLI on RHEL](https://docs.cloud.google.com/sdk/docs/install#rpm).
- You have created a Google Cloud instance using an AMD EPYC processor-based machine type from the supported list. For details, see [the specified Google Cloud instance types](https://docs.cloud.google.com/confidential-computing/confidential-vm/docs/supported-configurations#machine-type-cpu-zone).
...
<h2 id="idm139967677608208">Legal Notice</h2>
